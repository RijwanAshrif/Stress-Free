...
abstract class Database{
    ...
    public function not($column){
        if(count($column) == 1){
            if(empty(trim($this->sql))) $prepend = " WHERE";
            else $prepend = " AND ";

            $this->sql .= $prepend . " NOT " . key($column) . " = " . $this->formatSql($column[key($column)]);
        }
        return $this;
    }
    ...
    public function save($accepted_empty_key = []){
        $columns = "";
        $values = "";
        foreach($this->getVariables() as $key => $value){
            if(in_array($key, $accepted_empty_key)){
                $columns .= $key . ", ";
    ...
    public function count($distinct_column = null){
        if($this->is_sql) $this->sql =  "SELECT COUNT(*) " . $this->sql;
        else if($distinct_column == null) $this->sql = "SELECT COUNT(*) FROM " . strtolower(get_called_class()) . $this->sql;
        else $this->sql = "SELECT COUNT(DISTINCT " . $distinct_column . ") as count FROM " . strtolower(get_called_class()) . $this->sql;
    ...
    public function one($columns = null){
        ...
        }else if(!empty($columns)) $this->sql = "SELECT " . $columns . " FROM " . strtolower(get_called_class()) . $this->sql . $this->order_by . " LIMIT 1";
        else $this->sql = "SELECT * FROM " . strtolower(get_called_class()) . $this->sql . $this->order_by . " LIMIT 1";
        ...
        if(!empty($this->like)){
            foreach ($this->like as $inner_key => $inner_value){
                ...
                foreach($search_arr as $search_item){
                    if(!empty(trim($search_item))) $sql .= $this->sql . " AND " . $inner_key . " LIKE '%" . $search_item . "%' OR ";
                }
    ...
    public function all($column = null){
        if($this->is_sql){
            ...
            $this->sql = "SELECT " . $this->sql . $this->order_by . $this->limit;

        }else if(!empty($this->by_distance)){
    ...
    private function formatSql($var, $is_str = false){

        if($is_str){
            return "'" . $this->db_escape($var) . "'";
        }else{
